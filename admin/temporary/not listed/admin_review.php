...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['confirmBtn'])) {
        ...
        if ($preparedBy === $_SESSION['admin_name']) {
 ...
 elseif (isset($_POST['multipleCancelConfirmBtn']) && !empty($_POST['cancelledBy'])) {
        ...
        if (isset($_POST['selectedRows'])) {
            ...
            $updateQuery = "UPDATE soa_transaction SET status = 'Cancelled', reasonOf_cancellation = '$reasonOf_cancellation', cancelled_by = '$cancelledBy' WHERE reference_number IN ('" . implode("','", $selectedRows) . "')";
            if (mysqli_query($conn, $updateQuery)) {
        ...
        $updateQuery = "UPDATE soa_transaction SET status = 'Cancelled', reasonOf_cancellation = '$reasonOf_cancellation', cancelled_by = '$cancelledBy' WHERE reference_number = '$referenceNumber' AND status = ''";
        if (mysqli_query($conn, $updateQuery)) {
...
if (isset($_POST['EditConfirmBtn'])) {
    ...
    $sql = "UPDATE soa_transaction SET from_date = '$transactionFromDate', to_date = '$transactionToDate', number_of_transactions = '$numOfTransaction', amount = '$amount', add_amount = '$addAmount', vat_amount = '$vatAmount', net_of_vat = '$netOfVat', withholding_tax = '$wtax', net_amount_due = '$netAmountDue' WHERE reference_number = '$referenceNumber'";
    ...
    if ($stmt->affected_rows > 0) {
        ...
        $successMessage = "Selected row(s) Successfully Updated\<br>";
        displayModal($successMessage);
    } else {
...
<body>
   ...
   <div class="container">
      ...
      <center><h4>FOR REVIEW TRANSACTIONS</h4></center>
    <div class="data-table">
    <form action="" method="POST">    
    <div class="reviewed-btn">
        <button type="submit" id="forreviewed" class="reviewed" name="reviewed">For Review</button>
        <button type="button" id="forcancelled" name="cancelled">For Cancel</button>
    </div>
    <table>
        <tr>
            <!-- Table header code -->
        ...
        <?php while ($row = mysqli_fetch_assoc($result)): ?>
            <?php if (!empty($row['status'])) continue; ?>
            <?php
 ...
 <div id="confirmModal" class="modal">
    ...
    <input type="hidden" id="cancelledBy" name="cancelledBy" placeholder="Enter date" value="<?php echo $_SESSION['admin_name'] ?>">
            ...
            <div class="Edit-modal-content">
                ...
                <div class="edit-date">
                    ...
                    <div class="edit-transactionDate-div">
                        ...
                        <input class="edit-input" type="text" id="e-transactionFromDate" name="transactionFromDate" value="" >
                        <input class="edit-input" type="text" id="e-transactionToDate" name="transactionToDate" value="" >      
                    </div>

            ...
            if(is_readable($file)) {
                if($extension === 'xlsx' || $extension === 'xls') {
                    ...
                    try {
                        ...
                        echo '<script>
                            Swal.fire({
                                ...
                                window.location.href = "../../dashboard/billspayment/import/billspay-transaction-debug.php";
                            });
                        </script>';
                        exit;
                    }
                    ...
                    function checkSpelledRegionName($conn, $fileType, $region_description, $region_code = null) {
                        ...
                        if ($fileType === 'KP7') {
                            $query = "SELECT COUNT(*) as count FROM masterdata.region_masterfile WHERE (gl_region = ? OR region_desc_kp7 = ?) AND NOT zone_code IN ('VISMIN-MANCOMM', 'LNCR-MANCOMM', 'VISMIN-SUPPORT', 'LNCR-SUPPORT', 'HO') LIMIT 1";
                        } else {
                            $query = "SELECT COUNT(*) as count FROM masterdata.region_masterfile WHERE (gl_region = ? OR region_desc_kpx = ?) AND NOT zone_code IN ('VISMIN-MANCOMM', 'LNCR-MANCOMM', 'VISMIN-SUPPORT', 'LNCR-SUPPORT', 'HO') LIMIT 1";
                        }
                    ...
                    echo '<script>
                                Swal.fire({
                                    ...
                                    window.location.href = "billspay-transaction-debug.php";
                                });
                            </script>';
                }
            }
            ...
            if(is_readable($file)) {
                if($extension === 'xlsx' || $extension === 'xls') {
                    ...
                    if (!empty($_SESSION['consolidated_data'])) {
                        ...
                        echo '<script>window.location.href = "../error/ErrorResult-debug.php";</script>';

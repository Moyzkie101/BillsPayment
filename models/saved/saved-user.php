...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        function collect_leaf_keys($nodes)
        {
            $out = [];
            ...
            foreach ($nodes as $node) {
                ...
                if (isset($node['children']) && is_array($node['children']) && count($node['children'])) {
                    ...
                    if (isset($node['key']) && is_string($node['key'])) $out[] = $node['key'];
                }
            }
            $out = array_values(array_unique(array_filter($out, function($v){ return is_string($v) && trim($v) !== ''; })));
